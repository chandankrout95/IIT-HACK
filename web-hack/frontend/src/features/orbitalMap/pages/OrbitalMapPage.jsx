...
const OrbitalMapPage = () => {
  ...
  useEffect(() => {
    const fetchAsteroids = async () => {
      try {
        ...
        const asteroidsOnDate = response.data.near_earth_objects[selectedDate] || [];
        const mappedAsteroids = asteroidsOnDate.map((neo, i) => mapAsteroidData(neo, i, asteroidsOnDate.length));
  ...
  const mapAsteroidData = (neo, index, total) => {
    ...
    const avgMeters = (neo.estimated_diameter.meters.estimated_diameter_min + neo.estimated_diameter.meters.estimated_diameter_max) / 2;
...
const handleSearch = async (e) => {
  ...
  try {
    ...
    if (/[a-zA-Z\s]/.test(input)) {
      ...
      if (lookupRes.data && lookupRes.data.result && lookupRes.data.result.length > 0) {
    ...
    alert(`IDENTIFICATION FAILURE: "${input}" is too vague or not in the NEO database. Try adding a number (e.g., 2009 SY5) or use a 7-digit SPK-ID.`);
  } finally {
  ...
  const handleFocus = (pos, objData) => {
    ...
    controlsRef.current?.setLookAt(
      pos[0] + zoomDist, pos[1] + 1, pos[2] + zoomDist,
      pos[0], pos[1], pos[2],
      true
    );
  };
    ...
    <div className="absolute inset-0 bg-[#020202] text-white font-mono overflow-hidden">
      ...
      <div className="absolute top-8 left-8 z-20 pointer-events-none">
        <div className="bg-black/60 border-l-2 border-red-600 p-4 backdrop-blur-md">
          <h3 className="text-[9px] uppercase tracking-[0.4em] text-gray-500">Global NEO Telemetry</h3>
          <p className="text-sm font-black italic tracking-tighter uppercase flex items-center gap-2">
            {isLoading || isSearching ? <Loader2 className="animate-spin" size={14} /> : <Target size={14} className="text-red-500" />}
      ...
      <div className="absolute top-8 right-8 z-20 flex flex-col gap-4 w-64">
        ...
        {/* 🛰️ SEARCH BOX */}
        <form onSubmit={handleSearch} className="bg-black/60 border border-white/10 p-3 backdrop-blur-md">
          <div className="flex items-center gap-2 mb-2">
            <Search size={12} className="text-yellow-500" />
            <span className="text-[9px] uppercase tracking-widest text-gray-400">Target ID Search</span>
          </div>
          <div className="flex gap-1">
            ...
            />
            <button type="submit" className="bg-yellow-600 hover:bg-yellow-500 text-black px-3 font-bold text-[10px] transition-colors">
              {isSearching ? "..." : "GO"}
            </button>
          </div>
        </form>
        ...
        <div className="bg-black/60 border border-white/10 p-3 backdrop-blur-md flex flex-col gap-2">
          <div className="flex items-center gap-2 border-b border-white/5 pb-2 mb-1">
            ...
            <span className="text-[9px] uppercase tracking-widest text-gray-400">Temporal Filter</span>
          </div>
          <input 
            type="date" 
            value={selectedDate}
        ...
        <div className="bg-black/60 border border-white/10 p-3 backdrop-blur-md flex items-center justify-between">
          <div className="flex items-center gap-2">
            <ShieldAlert size={14} className={showOnlyHazardous ? "text-red-500 animate-pulse" : "text-gray-500"} />
            <span className="text-[9px] uppercase tracking-widest text-gray-400">Hazardous</span>
          </div>
          <button 
          ...
          >
            <div className={`absolute top-1 w-3 h-3 bg-white rounded-full transition-all ${showOnlyHazardous ? "left-6" : "left-1"}`} />
          </button>
        </div>
      </div>
      ...
      <div className="absolute bottom-8 left-8 z-20 flex gap-3">
        <button onClick={() => controlsRef.current?.setLookAt(0, 150, 200, 0, 0, 0, true)} className="px-6 py-3 bg-white/5 border border-white/10 hover:bg-white/10 text-[10px] uppercase font-bold tracking-widest flex items-center gap-2">
          <Globe size={14} className="text-blue-400" /> Galaxy View
        </button>
        <button onClick={() => { setSelectedObj(null); controlsRef.current?.setLookAt(40, 30, 50, 0, 0, 0, true); }} className="px-6 py-3 bg-white/5 border border-white/10 hover:bg-white/10 text-[10px] uppercase font-bold tracking-widest flex items-center gap-2">
      ...
      <Canvas camera={{ position: [50, 40, 80], fov: 40 }}>
        <Suspense fallback={null}>
          ...
          <pointLight position={[0, 0, 0]} intensity={5} distance={100} decay={1} color="#ffffff" />
          <group>
            <Sun onSelect={() => handleFocus([0, 0, 0], { name: "The Sun", size: 2.5 })} />
            {planetData.map((planet) => (
              <group key={planet.name}>
                <mesh rotation={[-Math.PI / 2, 0, 0]}>
                  <ringGeometry args={[planet.dist - 0.01, planet.dist + 0.01, 128]} />
                  <meshBasicMaterial color="white" opacity={0.05} transparent />
                </mesh>
                <MovingPlanet data={planet} onSelect={(pos) => handleFocus(pos, planet)} />
              </group>
            ))}
            {!isLoading && filteredAsteroids.map((asteroid) => (
              <Asteroid key={asteroid.id} {...asteroid} onSelect={(pos) => handleFocus(pos, asteroid)} />
            ))}
          </group>
